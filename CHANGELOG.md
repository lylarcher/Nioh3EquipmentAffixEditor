...
* **记录表改为「定位」而非「写死偏移」**：此前记录区起点是参考项目在
  ...
  * 判定沿用参考项目识别真实记录的条件：`type@0 == type@2`、`level@6 == level@8`、
    type 非 0，且为绘卷类型或 `item_count == 1`（`@4`）。最后一条很关键：空闲效果槽里
    是 `0xFFFFFFFF`，自身镜像，不加这条会把槽内字节误认成嵌套记录。
  * 命中项按「偏移 mod `0xE8`」聚类（同一张记录表共用一个余数），再按其后 400 槽中
    有效记录数确认对齐；随机字节命中概率约 `1/2^48`。
  * 定位失败**宁可失败也不乱写**：抛错并提示运行 `scan`，绝不按猜到的字节修改存档。
  * 记录表若仍在参考项目捕获的位置，槽号沿用该表自身编号（卖掉最前面的物品也不会让
    其它记录的槽号漂移）；若确实移动了，则以第一条已占用槽为锚点，并在报告里说明。

...
| 4 | 每槽关键词匹配词条 | 在**某一个槽**里输入关键词（或 `0x646b` / `646b` / 十进制 id），只有该槽的下拉列表被缩小；回车唯一命中即选中 | `edit --search <关键词>`, or `--edit 槽:关键词[:数值]` | **every** match is listed; 0 matches and >1 matches are both refused with the candidate list, so an ambiguous keyword can never silently pick one；空格分隔多个关键词＝必须同时命中（如 `星 恢复`）；种类 与 恩宠/套装 两个筛选互相级联，各自只列出对方结果里出现过的取值 |

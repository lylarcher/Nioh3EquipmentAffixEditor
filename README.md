...
* **等级 / +値** — 180 and 30 for both tabs, taken from `class_limits_for_record`; the
  ...
  used as a cap.
* **无中生有（实验性：需要进游戏实测确认）** — the button lives in its **own block at
  the bottom of the tab**, separated from the editor above, and 应用修改 never touches it:
  that button only rewrites the selected existing record. The 武器 / 防具 block can also
  set 稀有度 (blank = follow the template). The engine path exists and carries 34 tests,
  but a created item has not been confirmed in game yet. A same-kind real record is the
  template; when there is none, a same-小类 record is used and the slots that do not fit
  are cleared, with the reason shown before anything is written.
...
| 5 | 无中生有 | 无中生有 区块 on all four tabs（自己的独立框） | `create --kind … [--soul]` | a new record is copied from a **same-kind real sample**; 背包满 is refused with 「背包已满…请先在游戏里清理背包后再添加」 |
